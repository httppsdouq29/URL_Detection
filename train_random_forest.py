...
if __name__ == "__main__":
    ...
    # In kết quả
    print(f"\nRandom Forest : Accuracy on training Data: {acc_train:.3f}")
    print(f"Random Forest : Accuracy on test Data: {acc_test:.3f}")
    print()
    print(f"Random Forest : f1_score on training Data: {f1_train:.3f}")
    print(f"Random Forest : f1_score on test Data: {f1_test:.3f}")
    print()
    print(f"Random Forest : Recall on training Data: {recall_train:.3f}")
    print(f"Random Forest : Recall on test Data: {recall_test:.3f}")
    print()
    print(f"Random Forest : precision on training Data: {precision_train:.3f}")
    print(f"Random Forest : precision on test Data: {precision_test:.3f}")
    print()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('..', 'picture', 'confusion', 'confusion_matrix_random_forest.png'), bbox_inches='tight')
    plt.close(fig)
    ...
    ax.set_title('Top 10 đặc trưng quan trọng nhất - Random Forest')
    ax.set_xlabel('Mức độ quan trọng')
    ax.set_ylabel('Đặc trưng')
    plt.tight_layout()
    plt.savefig(os.path.join('..', 'picture', 'Feature', 'feature_importance_random_forest.png'), bbox_inches='tight')
    plt.close(fig)
    
    # Lưu mô hình
    os.makedirs(os.path.join('..', 'Model_PKL'), exist_ok=True)
    joblib.dump(model, os.path.join('..', 'Model_PKL', 'random_forest_model.pkl'))
    ...
    with open(os.path.join('..', 'model_metadata_random_forest.json'), 'w', encoding='utf-8') as f:
        json.dump({
